...
- **Interactive command outcome unknown:** the same idempotency key must keep returning `409 COMMAND_OUTCOME_UNKNOWN`, and the unresolved `needs_reconciliation` row intentionally fences every new control for that mission with `MISSION_COMMAND_BUSY`. Do not retry under a new key or clear the row blindly: close traffic, preserve SQLite, correlate the command phase/effect receipt with both the target and deterministic successor run, and prove the Hermes session/process inactive before an explicitly reviewed reconciliation. Indy never repeats an unproved interrupt, launch, or stop automatically.
