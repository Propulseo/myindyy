...
- La CI ordonne : install frozen, typecheck, tests TypeScript, tests Python, build, scan des clés de modèle, installation Chromium, E2E, rendu Compose, build Docker et inspection statique de l'image. Ses actions sont épinglées par commit. Le scan dérive les quatre noms interdits de `server/runtime/policy.ts` et bloque toute occurrence exacte, sans exclusion de dossier, dans chaque fichier texte suivi. Les seules références de politique, documentation, tests et contrôles de déploiement admises portent une suppression explicite sur leur ligne.
...
- `pnpm exec vitest run tests/model-key-scan.test.ts tests/hermes-runtime-manifest.test.ts tests/runtime-policy.test.ts` — 3 fichiers, 20 tests, 0 échec pour la revue 2/5.
- `pnpm test` — 31 fichiers, 250 tests, 0 échec.
...
- `pnpm test:e2e` — rebuild client/serveur puis 3 scénarios, 3 réussis en 9,4 s (passage final après tous les correctifs).
...
- `docker build --pull=false --tag indy-task11:review2 .` — passage définitif exit 0 en 14,6 s; manifest list `sha256:18161cf0212debf89706fd18445708c8ea10867dcb30f5e137189009586d6fe0`.
- `docker image inspect indy-task11:review2` — `User="indy:indy"`, healthcheck `healthcheck.js` (liveness), base/snapshot conformes et aucune variable de clé modèle.
...
- Un commit correctif atomique ferme les findings de revue 2/5 décrits ci-dessus; son hash est communiqué avec le statut final.
