...
- TOCTOU RED : le bind validé restait la source Python exécutée et pouvait être retargeté côté hôte. GREEN intermédiaire : le manifeste externe était copié dans le scratch, le bind validé puis copié dans `mkdtemp`, et la copie revalidée avant le premier import. L'env enfant pointait seulement vers la copie, retirait les trois chemins Python injectables et forçait les modes safe/no-user-site. La revue 5/5 ci-dessous remplace le snapshot de manifeste mutable par l'ancre externe originale.
...
### Revue corrective 5/5

- Ancre RED : le manifeste revu était copié dans le scratch possédé par UID `10001`; ce même UID pouvait altérer le runtime privé puis écrire un manifeste forgé cohérent. GREEN : aucun manifeste n'entre désormais dans le scratch. L'entrypoint résout le config original, exige en production un fichier régulier UID 0 sans bit d'écriture, puis valide source et copie privée contre ce même chemin externe. Le serveur parent du worker capture et gèle `{runtimeRoot, manifestAnchor, python}` à l'import; spawn et readiness réutilisent ce contrat, et l'environnement enfant réécrit toute tentative de substitution du chemin d'ancre. Le repro altère les bytes privés, crée un faux manifeste privé et injecte son chemin : le gate échoue sur le hash attendu par l'ancre originale.
- Scanner RED : des noms interdits encodés par escapes littéraux JavaScript/YAML ou construits par `String.fromCharCode` donnaient zéro finding. GREEN : les vues mappées décodent `\\uNNNN`, `\\xNN` et `\\u{...}` avant normalisation, puis replient les appels `String.fromCharCode` à arguments entiers constants dans les accès `process.env`. Les trois familles de bypass sont générées par codepoints dans les tests, sans inscrire les noms sensibles en clair dans le dépôt.
- Compose/Docker : le config manifeste est déclaré `uid: "0"`, `gid: "0"`, `mode: 0444`; l'image précrée `/run/indy-config` root-owned `0555`. La CI crée un conteneur avec l'utilisateur image, copie une fixture comme UID 0, puis exige UID effectif `10001`, propriétaire du fichier `0` et `test ! -w` avant de poursuivre.
...
- `pnpm test` — revue 5/5 : 31 fichiers, 257 réussis et 2 tests POSIX différés sous Windows, 0 échec.
...
- `pnpm test:e2e` — rebuild client/serveur puis 3 scénarios, 3 réussis en 17,6 s après la revue 5/5.
...
- `docker build --tag indy-task11-round5 .` — passage définitif exit 0; image Linux `sha256:506085b676ed684032df44ba7196cf7e6edf1f4e93bd9d60ec608ec251df191b`.
- `docker run --user 10001:10001 ... vitest ... tests/container-entrypoint.test.ts tests/model-key-scan.test.ts` — 19/19 réussis sous Linux, y compris refus d'une ancre production possédée/inscriptible par le service, modes privés et bypass scanner. Un run plus large a correctement signalé que l'étage build Node ne contient pas Python; la vérification Python est donc effectuée dans l'image runtime, où `python3 -I` confirme UID `10001` et mode isolé.
- conteneur jetable image finale avec fichier copié dans `/run/indy-config` — UID effectif `10001`, ancre UID `0`, non inscriptible; `User="indy:indy"`, healthcheck liveness attendu et scan générique d'environnement verts.
...
- `c4d14fe` — `fix: harden Task 11 runtime execution`.
- Un commit correctif atomique ferme les findings de revue 5/5 décrits ci-dessus; son hash est communiqué avec le statut final.
...
- L'ancre est immuable pour le processus non-root et n'est jamais choisie depuis un RPC worker; un administrateur host/root reste cependant dans la base de confiance et peut remplacer un Docker config entre deux validations. Le rollout doit donc conserver les contrôles de digest/provenance et les droits root-only documentés.
- Le scanner replie les encodages littéraux et constructions constantes demandés, mais n'est pas un interpréteur JavaScript/shell général. Une construction calculée à partir d'entrées dynamiques sort de son analyse statique; la seconde barrière reste le rejet générique des quatre entrées dans l'environnement réel et leur suppression avant spawn.
