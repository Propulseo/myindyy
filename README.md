...
Hermes is the only source of schedule truth and the only component that decides when a recurring task runs. Indy does not copy or rewrite schedules and does not run a parallel ticker. The Indy-owned Hermes worker installs a controlled hook in Hermes's shared per-job execution path: immediately before every automatic or manual occurrence it refreshes OAuth/catalog state and applies the policy below. A refused job never reaches inference and Hermes records the refusal as a failed occurrence. The hook also disables provider/model fallback only in that admitted cron execution context.
...
Each completed occurrence writes a redacted output artifact followed by an immutable terminal manifest under `$HERMES_HOME/cron/indy-manifests/<task-id>/`. The manifest contains the Hermes execution ID, original task identity and runtime snapshot, exact start/finish timestamps, status/error, output reference and optional manual dispatch token. Indy passively imports only these terminal manifests at startup and while running; partial/temp/raw output files are deferred, deleted jobs remain importable, and the importer never calls the scheduler or a trigger.
...
Manual runs use the operator command table as a durable outbox. The idempotency claim happens before task/runtime lookup, unresolved duplicates truthfully return `202 pending`, and startup resumes claimed commands only after fresh policy validation. Hermes persists the dispatch token in the same locked `jobs.json` mutation as its run-now intent, so crash-before resumes and crash-after replays the receipt without a second trigger. The HTTP acknowledgement remains pending until a terminal manifest bearing that exact token appears; the UI never correlates an unrelated automatic or legacy raw run.
