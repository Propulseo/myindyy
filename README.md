...
The guarded execution integration is pinned to `hermes-agent==0.15.1`. Startup verifies both that exact installed version and the scheduler source contract that creates a `cron.executions` record, copies its ID through the claimed job, and passes that same job through `run_one_job` / `_run_one_job_body` to `run_job`. To upgrade Hermes, first update the supported-version constant only after reviewing those scheduler functions, run the installed-contract and simulated-drift Python tests, then run the full Python/TypeScript suites, typecheck and build. A version, source or signature mismatch intentionally leaves the worker unavailable rather than executing an unguarded cron.
